...
const VideoPlayer = ({ video, selectedVideo }) => {
  ...
  const fetchStreamUrl = useMemo(
    () => async (videoUrl) => {
      ...
      const apiUrl = `${import.meta.env.VITE_BACKEND_URL}/api/v1/stream`;
  ...
  const VideoElement = useMemo(() => {
    ...
    return (
      <>
        ...
        <audio className="pointer-events-none w-0 h-0 opacity-0">
          <source src={streamUrlRef.current} type="audio/mp3" />
        </audio>
      </>
    );
