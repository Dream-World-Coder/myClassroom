...
const AddCoursePage = () => {
    ...
    const saveCourse = () => {
        ...
        const apiUrl =
            "http://127.0.0.1:5050/api/v1/extract-videos-from-playlist-url";
        const options = {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ courseUrl: courseUrl }),
        };

        // if courseUrl chnages then only fetch else save only
        setIsLoading(true);
        fetch(apiUrl, options)
            .then((res) => {
                if (!res.ok) {
                    toast.error(
                        "Some Error Occurred, please check the provided url.",
                    );
                    throw new Error(
                        "Some Error Occurred, please check the provided url.",
                    );
            ...
            .then((data) => {
                ...
                setPlaylistVideoList(data.playListData);
                const courseData = {
                    name: courseName,
                    ...
                    >
                        {previewCourse ? (
                            <CourseCard
                                ...
                                courseThumbnail={
                                    playlistVideoList[0].videoThumbnailUrl
                                }
                                progress={""}
                            />
                        ) : (
