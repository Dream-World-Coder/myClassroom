...
def extract_videos_from_url():
  ...
  if "youtube.com/playlist" not in course_url:
  ...
  try:
    ...
    if user:
      ...
      return jsonify({
        "message": "Course saved/updated successfully!",
...
def get_stream_url_decent():
  ...
  ydl_opts = {
    "format": "worst[height<=480]/worst",  # More specific low quality
    "quiet": True,
    "no_warnings": True,
...
def set_video_state():
  ...
  if request.method == "POST":
    ...
    return jsonify({'error': 'Method not allowed, only GET/POST'}), 405
